...
class BasicReadingUnit(object):
    ...
    def __init__(self, states, seq_length, init_cell_state, no_units, shared_attention, fuse_mode="dot", keep_dropout=1., feedback_loop=False, sum_memory=False, reuse=False):
...
class BasicReadingUnit2(object):
    ...
    def __init__(self, states, seq_length, init_cell_state, no_units, shared_attention, fuse_mode="dot", gating="relu", feedback_loop=False, keep_dropout=1., sum_memory=False, reuse=False):
        self.memory_cell = init_cell_state
        self.states = states
        self.seq_length = seq_length
        self.no_units = no_units
...
class BasicReadingUnit3(object):
    ...
    def __init__(self, states, seq_length, init_cell_state, no_units, shared_attention, fuse_mode="dot", feedback_loop=False, gating="relu", keep_dropout=1., sum_memory=False, reuse=False):
        self.memory_cell = init_cell_state
        self.states = states
        self.seq_length = seq_length
        self.no_units = no_units
...
class NoMemoryReadingUnit(object):
    ...
    def forward(self, image_feat):
        ...
        output = tfc_layers.layer_norm(output, reuse=self.reuse)
        #output = tf.layers.dropout(output, self.keep_dropout)
...
class ModulatingReadingUnit(object):
    ...
    def forward(self, image_feat):
        ...
        with tf.variable_scope("feedback_loop", reuse=self.reuse):
            ...
            no_state_feat = int(self.memory_cell.get_shape()[-1])

            film_params = tfc_layers.fully_connected(image_feat,
                                               num_outputs=2 * no_state_feat,
                                               activation_fn=None,
                                               reuse=self.reuse,
                                               scope="film_projection")

            gammas = film_params[:, :no_state_feat]
            betas = film_params[:, no_state_feat:]

            self.memory_cell = (1 + gammas) * self.memory_cell + betas
        ...
        # output = tf.layers.dropout(self.memory_cell, self.keep_dropout)

        return output


class ModulatingReadingUnit2(object):
    ...
    def forward(self, image_feat):
        ...
        with tf.variable_scope("feedback_loop", reuse=self.reuse):
            ...
            no_state_feat   = int(self.states.get_shape()[-1])
            no_state_length = tf.shape(self.states)[-2]
            ...
            film_params = tf.tile(film_params, [1, no_state_length, 1])

            gammas = film_params[:, :, no_state_feat]
            betas = film_params[:, :, no_state_feat:]

            states = (1 + gammas) * self.states + betas
        ...
        with tf.variable_scope(self.scope, reuse=reuse) as scope:
            ...
            new_memory_cell = attention.compute_attention(states,
                                                           seq_length=self.seq_length,
                                                           context=self.memory_cell,
                                                           no_mlp_units=self.no_units,
                                                           fuse_mode=self.fuse_mode,
...
def create_reading_unit(last_state, states, seq_length, config, keep_dropout, reuse):
    ...
    if unit_type == "only_question":
        ...
        return ModulatingReadingUnit(states=states,
                                 seq_length=seq_length,
                                 init_cell_state=last_state,
                                 no_units=no_units,
                                 shared_attention=shared_attention,
        ...
        return ModulatingReadingUnit2(states=states,
                                 seq_length=seq_length,
                                 init_cell_state=last_state,
                                 no_units=no_units,
                                 shared_attention=shared_attention,
